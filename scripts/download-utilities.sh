...
function download() {
  ...
  # Ensure we have curl
  verify_exec "${UBIRD_CURL}" 'UBIRD_CURL' || exit 1
  ...
  if [[ -z "${UBIRD_CURL_FLAGS+x}" ]] || [[ "${UBIRD_CURL_FLAGS}" == "" ]]; then
    echo_red_text "ERROR: 'UBIRD_CURL_FLAGS' is missing!"
    exit 1
  fi

  local -r url="$1"
  local -r output_file="$2"
  local -r file_name=$("${UBIRD_BASENAME}" "${output_file}")
  local -r file_dir=$("${UBIRD_DIRNAME}" "${output_file}")
  ...
  "${UBIRD_CURL}" ${UBIRD_CURL_FLAGS} --location "${url}" --output "${output_file}" || local download_failed=1
...
function clone_git_repo() {
  ...
  if [[ "${revision}" == 'no-revision' ]]; then
    if [[ "${submodules}" == 1 ]]; then
      "${UBIRD_GIT}" clone "${url}" "${path}" --depth="${depth}" --recurse-submodules || local clone_failed=1
    else
      "${UBIRD_GIT}" clone "${url}" "${path}" --depth="${depth}" || local clone_failed=1
    fi
  else
    if [[ "${submodules}" == 1 ]]; then
      "${UBIRD_GIT}" clone "${url}" "${path}" --depth="${depth}" --revision="${revision}" --recurse-submodules || local clone_failed=1
    else
      "${UBIRD_GIT}" clone "${url}" "${path}" --depth="${depth}" --revision="${revision}" || local clone_failed=1
    fi
  fi
