...
function create_archive() {
  ...
  local -r output_archive_dir="$("${UBIRD_DIRNAME}" "${output_archive}")"
  if [[ ! -d "${output_archive_dir}" ]]; then
    "${UBIRD_MKDIR}" -p "${output_archive_dir}"
  fi

  # If we're on OS X, clean the target directory
  if [[ "${UBIRD_OS}" == 'osx' ]]; then
    "${UBIRD_DOT_CLEAN}" -mv "${target_dir}"
  fi

  # Set the file timestamp
  ## (This is derived from UBIRD_VERSION_DATE at `versions.sh`)
  local -r UBIRD_STAMP="${UBIRD_VERSION_DATE//./-}"
  local -r UBIRD_FIND_STAMP="$("${UBIRD_DATE}" -d "${UBIRD_STAMP}" +"%a, %d %b %Y %T %z")"
  local -r UBIRD_TIMESTAMP="$("${UBIRD_DATE}" -d "${UBIRD_STAMP}" +"%Y-%m-%dT%H:%M:%SZ")"

  # Override the timestamps for each file to match our stamp above
  "${UBIRD_FIND}" "${target_dir}" -newermt "${UBIRD_FIND_STAMP}" -print0 |
    "${UBIRD_XARGS}" -0r "${UBIRD_TOUCH}" -h -d "${UBIRD_TIMESTAMP}"

  # Override the timestamps for each directory to match our stamp above
  for dir in $("${UBIRD_FIND}" "${target_dir}" -type d); do
    "${UBIRD_TOUCH}" -r "${dir}/$("${UBIRD_LS}" -At "${dir}" | "${UBIRD_HEAD}" -n 1)" "${dir}"
  done
  ...
  if [[ "${archive_format}" == 'zip' ]]; then
    ...
    "${UBIRD_ZIP}" -X -r "${output_archive}" * -x '.DS_Store' || local archive_failed=1
...
function extract_archive() {
  ...
  "${UBIRD_MKDIR}" -p "${temp_archive_path}"
  ...
  if [[ "${archive_format}" == 'zip' ]]; then
    "${UBIRD_UNZIP}" -q "${archive_path}" -d "${temp_archive_path}" || local extraction_failed=1
  elif [[ "${archive_format}" == 'tar.gz' ]]; then
    "${UBIRD_TAR}" xzf "${archive_path}" -C "${temp_archive_path}" || local extraction_failed=1
  elif [[ "${archive_format}" == 'tar.xz' ]]; then
    "${UBIRD_TAR}" xJf "${archive_path}" -C "${temp_archive_path}" || local extraction_failed=1
  ...
  "${UBIRD_CP}" -rf "${temp_archive_path}/${top_input_dir}/" "${target_path}" || local extraction_failed=1
