...
# Lints uBird's shell scripts with shellcheck (static analysis) and shfmt
...
readonly UBIRD_LINTING=1
export UBIRD_LINTING
...
if [[ "${mode}" == 'staged' ]]; then
  while IFS= read -r file; do
    [[ -n "${file}" ]] && targets+=("${file}")
  done < <("${UBIRD_GIT}" diff --cached --name-only --diff-filter=ACM -- 'scripts/*.sh')
else
  while IFS= read -r file; do
    targets+=("${file}")
  done < <("${UBIRD_LS}" scripts/*.sh)
fi
...
if ! "${UBIRD_SHELLCHECK}" -x "${targets[@]}"; then
  status=1
fi

echo 'lint: shfmt formatting check...'
if ! "${UBIRD_SHFMT}" -d "${targets[@]}"; then
  echo >&2
